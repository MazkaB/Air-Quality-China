...
def main():
    ...
    st.write("""
        **Selamat datang!** Dashboard ini menampilkan **beberapa visualisasi interaktif** 
        ...
        (tanggal, jam, stasiun, rentang PM2.5, rentang PM10) serta memilih **kolom** untuk 
        visualisasi _Histogram_, _Time Series_, _Scatter Plot_, dan _Bar Chart_.
    """)
    
    if "df" not in st.session_state:
        file_path = r"D:\submission\dashboard\main_data.csv"  # Use raw string for Windows path
        with st.spinner("Memuat data..."):
            df = load_data(file_path)
        if df.empty:
            st.error("Gagal memuat data. Pastikan file path dan file CSV sesuai.")
            return
    ...
    # Hour range filter (new interactive feature)
